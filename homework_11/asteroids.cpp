...
public:
            ...
            sExplosion_ship(t7, 0, 0, 192, 192, 64, 0.5)
    {
        ...
        {
            auto a = std::make_shared < Asteroid >(rng, uniform_int_distribution_asteroid, sRock,
                                                   uniform_int_distribution(rng) % constants::kWidth,
        ...
        p = std::make_shared < Player >(sPlayer, constants::kWidth / 2, constants::kHeight / 2, 0, player_radius);
            ...
            for (const auto b : entities)
            {
                if (a->name == constants::EntityName::Asteroid && b->name == constants::EntityName::Bullet)
                    if (isCollide(a, b))
                    {
                        ...
                        auto e = std::make_shared < Entity >(sExplosion, a->x, a->y);
                        ...
                        for (int i = 0; i < num_small_rocks_after_crash; i++)
                        {
                            ...
                            auto new_e = std::make_shared < Asteroid >(rng,uniform_int_distribution_asteroid,
                                                                       sRock_small, a->x, a->y,
                ...
                if (a->name == constants::EntityName::Player && b->name == constants::EntityName::Asteroid)
                    if (isCollide(a, b))
                    {
                        ...
                        auto e = std::make_shared < Entity >(sExplosion_ship, a->x, a->y);
                        e->name = constants::EntityName::Explosion;
                        entities.push_back(e);

                        p->set_parameters(sPlayer, constants::kWidth / 2, constants::kHeight / 2, 0, player_radius);
                        p->dx = 0; p->dy = 0;
                        p->lives--;
                    }
            }
    ...
    void draw(std::shared_ptr<Player> p, sf::Sprite background) {
        app.draw(background);
    ...
    void add_asteroid() {
        ...
        {
            auto a = std::make_shared < Asteroid >(rng, uniform_int_distribution_asteroid, sRock, 0,
                                                   uniform_int_distribution(rng) % constants::kHeight,
                                                   uniform_int_distribution(rng) % 360,
                                                   big_rock_radius);
            entities.push_back(a);
        ...
        while (app.isOpen() && p->is_alive())
        {
            ...
            while (app.pollEvent(event))
            {
                ...
                if (event.type == sf::Event::KeyPressed)
                    ...
                    {
                        auto b = std::make_shared < Bullet >(sBullet, p->x, p->y, p->angle, bullet_radius);
...
private:
    ...
    std::uniform_int_distribution<int> uniform_int_distribution_asteroid{-4, 4};

    static inline const unsigned int max_num_rocks = 15U;
    static inline const float big_rock_radius = 25.0f;
    static inline const float small_rock_radius = 15.0f;
    static inline const float player_radius = 20.0f;
    static inline const float bullet_radius = 10.0f;
    static inline const unsigned int num_small_rocks_after_crash = 2U;
};
